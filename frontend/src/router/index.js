...
const routes = [
    ...
    },
    {
        ...
        beforeEnter: () => {
            ...
            if (!globalConfigStore.globalConfig?.allowRegistration)
                return '/';
        }
    },
    {
...
async function getRedirect(to) {
    const authStore = useAuthStore();
    if (to.meta.denyAccessAsLoggedIn && authStore.isLoggedIn)
        return Promise.resolve('/');
    if (to.name === 'Admin' && !authStore.isLoggedIn)
        return Promise.resolve('/login');
    return Promise.resolve(undefined);
}
router.beforeEach(async (to, from, next) => {
    const redirect = await getRedirect(to);
    if (redirect)
        return next(redirect);
    next();
});
router.afterEach((to) => {
    useTitle({ title: to.meta?.title ?? '' });
});
