...
const fetchData = async (query, page) => {
  try {
    const data = await getData({ endpoint: `/search/tv?query=${query}&page=${page}` })
...
const navigate = pageNumber => { router.push({ path: '/search/tv', query: { ...route.query, page: pageNumber } }) }

onBeforeMount(()=>{
  fetchData(route.query.q, route.query.page);
})
</script>
